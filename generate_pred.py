...
def formatLiterals(variableSet, joiner):
    ret = joiner.join([ "L" + x for x in variableSet ])
    return ret

# Format clauses into Forge code.
def formatClause(clauseSet, joiner):
    ret = joiner.join([ "C" + str(x.id) for x in clauseSet ])
    return ret

# Format one literal assignment into Forge code.
def formatOne(clauseId, literal):
    sign = "True" if literal.sign else "False"
    return "C" + str(clauseId) + "->L" + literal.name + "->" + sign
...
if __name__ == "__main__":
    ...
    print("pred generatedCaseX {")
    print(" "*4 + "some " + formatLiterals(variableSet, ", ") + " : Literal | {")
    print(" "*8 + "#(" + formatLiterals(variableSet, " + ") + ") = " + str(len(variableSet)))
    print(" "*8 + "some " + formatClause(clauseSet, ", ") + " : Clause | {")
    print(" "*12 + "#(" + formatClause(clauseSet, " + ") + ") = " + str(len(clauseSet)))
    print(" "*12 + formatLitset(clauseSet))
    print(" "*8 + "}")
    print(" "*4 + "}")
    print("}")